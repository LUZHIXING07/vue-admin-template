...
export default {
  ...
  methods: {
    // 自定义事件的回调
    getCategoryId({ catgoryId, level }) {
      // 区分三级分类相应的id,一级父组件进行存储
      if (level == 1) {
        this.catgory1Id = catgoryId;
        this.catgory2Id = "";
        this.catgory3Id = "";
      } else if (level == 2) {
        this.catgory2Id = catgoryId;
        this.catgory3Id = "";
      } else {
        // 代表三级分类的id有了
        this.catgory3Id = catgoryId;
        // 发送请求获取品牌属性
        this.getAttrList();
      }
    },
    // 发送请求获取平台属性数据
    // 当用户确定三级分类的数据的时候,可以向服务器发送请求获取 平台属性数据
    async getAttrList() {
      // 获取分类的ID
    ...
    addAttr() {
      ...
      this.attrInfo = {
        ...
        categoryId: this.catgory3Id, //三级分类的id
        categoryLevel: 3, //因为服务器也需要区分几级id
      };
    },
    // 修改某一个属性
    ...
    toLook(row) {
      // 如果属性值为空不能作为新的属性值,需要给用户提示:请输入一个属性值
