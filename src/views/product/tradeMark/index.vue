...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        // 品牌名称的验证规则
        // require:必须要校验字段(和前面的五角星有关系)
        // message:提示信息   trigger:用户行为设置(事件的设置:blur/change)
        tmName: [
          {
            required: true,
            message: "请输入品牌名称",
  ...
  methods: {
    ...
    deleteTradeMark(row) {
      // 弹框
      this.$confirm(`此操作将永久删除${row.tmName},是否继续吗?`, "提示", {
    ...
    handleAvatarSuccess(res, file) {
      ...
      console.log("上传图片成功之后返回前端的数据:", res);
